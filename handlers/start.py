...
from utils.vpn_manager import VPN_USERS_FILE, load_vpn_db, format_size
...
async def admin_check_expiry(callback: types.CallbackQuery):
    ...
    # Получаем данные о бессрочных конфигах
    vpn_users = load_vpn_db()
    permanent_count = 0
...
async def admin_stats(callback: types.CallbackQuery):
    """Полная статистика с конфигами, прокси и последним подключением"""
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("⛔ Доступ запрещен", show_alert=True)
        return
    ...
    from utils.vpn_manager import get_user_traffic_from_clients_table, format_size
    traffic_data = get_user_traffic_from_clients_table()
    
    # Загружаем данные о конфигах и прокси
    vpn_users = load_vpn_db()
    user_proxies = load_json(USER_PROXIES_FILE, {})
    ...
    # 1. Из vpn_users.json — количество конфигов
    ...
    # 2. Из user_proxies.json — количество прокси
    for user_id_str, data in user_proxies.items():
        user_id = int(user_id_str)
        proxies = data.get('proxies', [])
        # Ищем username по user_id
        username = None
    ...
    # 4. Из traffic_data — трафик и последнее подключение
    for username, traffic in traffic_data.items():
        if username in users_info:
            ...
            # Новый пользователь из traffic_data
            users_info[username] = {
                'user_id': None,
                'vpn_configs': 0,
                'proxies': 0,
